...
- `DB` — D1 database `testllm-d1`

`testllm-r2` also exists (created for naming-scheme parity) but is not bound here — there is no upload feature in this app yet.
...
**D1 schema** (`testllm-d1`) is not tracked anywhere in this repo (no migrations directory) — it exists only as tables referenced ad hoc from `src/index.ts`: `users`, `messages`, `memories`, `shared_knowledge`. Any schema change has to be applied directly against the D1 database and is invisible to git history.

**This app is TestLLM, not part of the ServTech brand.** It was briefly named "ServTech-v2" — that was wrong and has been reverted; this app is unrelated to and independent from the ServTech product line.

**Do not point this Worker's `DB` binding at `servtech-d1` (formerly `servtechdb`, formerly `warnetworkllm-db`).** That database backs the separate, authenticated `ServTech` app (`servtech-worker` — password accounts, sessions, uploads). This app's schema is incompatible with it (this app expects `username`/`session_id` columns; the real one uses `user_id` foreign keys) and its identity model has no auth at all, so sharing the database would let anyone read/write another app's user data through a `?u=<guessed-username>` query param. This app has its own dedicated, isolated database (`testllm-d1`) — keep it that way.
