...
- `DB` — D1 database `servtechllmv2db`

`servtechllmv2r2` also exists (created for naming-scheme parity with the rest of ServTech-v2) but is not bound here — there is no upload feature in this app yet.
...
**D1 schema** (`servtechllmv2db`) is not tracked anywhere in this repo (no migrations directory) — it exists only as tables referenced ad hoc from `src/index.ts`: `users`, `messages`, `memories`, `shared_knowledge`. Any schema change has to be applied directly against the D1 database and is invisible to git history.

**Do not point this Worker's `DB` binding at `servtechdb` (formerly `warnetworkllm-db`).** That database backs the separate, authenticated `ServTech` app (`servtechworker` — password accounts, sessions, uploads). This app's schema is incompatible with it (this app expects `username`/`session_id` columns; the real one uses `user_id` foreign keys) and its identity model has no auth at all, so sharing the database would let anyone read/write another app's user data through a `?u=<guessed-username>` query param. This app has its own dedicated, isolated database (`servtechllmv2db`) — keep it that way.
